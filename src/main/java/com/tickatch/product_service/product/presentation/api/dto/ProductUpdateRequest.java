 ...
 * 상품 수정 요청 DTO.
 *
 * <p>기존 상품의 정보 수정에 필요한 데이터를 담는다. 모든 필드는 선택적이며, null이 아닌 필드만 수정된다. 단, 세트로 묶인 필드들은 모두 함께 제공해야 한다.
 *
 * <p>세트 필드:
 *
 * <ul>
 *   <li>일정: startAt, endAt, saleStartAt, saleEndAt (4개 모두 필요)
 *   <li>장소: stageId, stageName, artHallId, artHallName, artHallAddress (5개 모두 필요)
 *   <li>좌석: seatGradeInfos + seatCreateInfos (둘 다 필요, 전체 교체)
 * </ul>
 *
 * @author Tickatch
 * @since 1.0.0
...
public record ProductUpdateRequest(
    // ========== 기본 정보 (개별 수정 가능) ==========
    @Schema(description = "상품명", example = "레미제라블") @Size(max = 50, message = "상품명은 50자 이하여야 합니다")
        String name,
    @Schema(description = "상품 타입", example = "MUSICAL") ProductType productType,
    @Schema(description = "상영 시간 (분)", example = "150") @Positive(message = "상영 시간은 양수여야 합니다")
        Integer runningTime,

    // ========== 일정 (세트 - 4개 모두 필요) ==========
    @Schema(description = "행사 시작 일시", example = "2025-03-01T19:00:00") LocalDateTime startAt,
    @Schema(description = "행사 종료 일시", example = "2025-03-01T21:30:00") LocalDateTime endAt,
    @Schema(description = "예매 시작 일시", example = "2025-02-01T10:00:00") LocalDateTime saleStartAt,
    @Schema(description = "예매 종료 일시", example = "2025-02-28T23:59:59") LocalDateTime saleEndAt,

    // ========== 장소 (세트 - 5개 모두 필요) ==========
    @Schema(description = "스테이지 ID", example = "1") Long stageId,
    @Schema(description = "스테이지명", example = "올림픽홀") String stageName,
    @Schema(description = "공연장 ID", example = "100") Long artHallId,
    @Schema(description = "공연장명", example = "올림픽공원") String artHallName,
    @Schema(description = "공연장 주소", example = "서울시 송파구 올림픽로 424") String artHallAddress,
    ...
    // ========== 관람 제한 (개별 수정 가능) ==========
    @Schema(description = "관람 등급", example = "ALL") AgeRating ageRating,
    ...
    // ========== 환불 정책 (개별 수정 가능) ==========
    @Schema(description = "취소 가능 여부", example = "true") Boolean cancellable,
    ...
    // ========== 좌석 (세트 - 전체 교체) ==========
    @Schema(description = "좌석 등급 정보 리스트 (전체 교체)") @Valid List<SeatGradeRequest> seatGradeInfos,
    @Schema(description = "개별 좌석 정보 리스트 (전체 교체)") @Valid List<SeatCreateRequest> seatCreateInfos) {

  /**
   * Compact constructor로 방어적 복사 수행. List 필드를 불변 리스트로 변환하여 내부 표현 노출을 방지한다. null인 경우는 null을 유지한다 (선택적
   * 필드이므로).
   */
  public ProductUpdateRequest {
    seatGradeInfos = seatGradeInfos == null ? null : List.copyOf(seatGradeInfos);
    seatCreateInfos = seatCreateInfos == null ? null : List.copyOf(seatCreateInfos);
  }
  ...
  public record SeatGradeRequest(
      @Schema(description = "등급명", example = "VIP") String gradeName,
      @Schema(description = "가격", example = "150000") @PositiveOrZero(message = "가격은 0 이상이어야 합니다")
          Long price,
      @Schema(description = "총 좌석 수", example = "100") @Positive(message = "총 좌석 수는 양수여야 합니다")
          Integer totalSeats) {}

  /** 개별 좌석 정보 요청. */
  @Schema(description = "개별 좌석 정보")
  public record SeatCreateRequest(
      @Schema(description = "좌석번호", example = "A1") String seatNumber,
      @Schema(description = "등급명", example = "VIP") String grade,
      @Schema(description = "가격", example = "150000") @PositiveOrZero(message = "가격은 0 이상이어야 합니다")
          Long price) {}
}
