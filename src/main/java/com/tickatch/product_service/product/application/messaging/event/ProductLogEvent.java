 ...
 * 상품 로그 이벤트.
 *
 * <p>상품 도메인에서 발생하는 주요 액션에 대한 로그 정보를 담는 이벤트 객체이다. 로그 서비스로 전송되어 상품 관련 활동 이력을 기록한다.
 *
 * <p>이벤트 정보:
 *
 * <ul>
...
public record ProductLogEvent(
   ...
   * 새로운 상품 로그 이벤트를 생성한다.
   *
   * <p>eventId와 occurredAt은 자동으로 생성되고, actorType과 actorUserId는 {@link ActorExtractor}를 통해
   * SecurityContext에서 자동 추출된다.
   *
   * @param productId 대상 상품 ID
   ...
   *
   * <p>스케줄러나 내부 시스템 작업 등 사용자 컨텍스트가 없는 경우 사용한다. actorType은 "SYSTEM"으로, actorUserId는 null로 설정된다.
   *
   * @param productId 대상 상품 ID
   * @param actionType 액션 타입
   * @return 생성된 ProductLogEvent
  ...
  public static ProductLogEvent createSystemEvent(Long productId, String actionType) {
    return new ProductLogEvent(
        UUID.randomUUID(), productId, actionType, "SYSTEM", null, LocalDateTime.now());
  }
}
